...
set TOP DA4ML_Top
set N_SIZE [expr {[info exists ::env(CURRENT_N)] ? $::env(CURRENT_N) : 8}]
set CLK_PERIOD 5.0
set RTL_DIR ./RTL
...
# 中文修正：z_buffer 的 D 由主時脈 FF 同緣送出；若目的 FF 改用延遲後的
# gated clock，gate simulation 會形成約 25ps race。只排除 z_buffer bank，
# 其餘暫存器仍允許 latch clock gating，不能用 +notimingcheck 掩蓋此問題。
set ZBUF_REGS [get_cells -hierarchical *z_buffer_reg*]
if {[sizeof_collection $ZBUF_REGS] == 0} {set ZBUF_REGS [get_cells -hierarchical *z_buffer*]}
set ZBUF_COUNT [sizeof_collection $ZBUF_REGS]
if {$ZBUF_COUNT == 0} {fatal "Cannot find z_buffer registers for clock-gating exclusion"}
if {[llength [info commands set_clock_gating_objects]] == 0} {fatal "set_clock_gating_objects is unavailable"}
set_clock_gating_objects -exclude $ZBUF_REGS
puts "INFO: Excluded $ZBUF_COUNT z_buffer registers from clock gating"

if {[file exists ./DA4ML_Top.saif]} {catch {read_saif -input ./DA4ML_Top.saif -instance_name TB_DA4ML_16x16/U_DUT -auto_map_names}}
if {[catch {compile_ultra -gate_clock} err]} {fatal "compile_ultra failed: $err"}
